...
export default async function Discover({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  <section className="grid hunterGrid">{items.map(p=>{const cost=parsePrice(p.nowPrice||p.sellPrice);const sell=suggestedPrice(cost);const profit=sell-cost;const margin=sell?profit/sell*100:0;const score=scoreProduct(p);return <article className="card" key={p.id}><div className="visual liveVisual"><span className="tag">CJ LIVE</span>{p.bigImage?<img src={p.bigImage} alt={p.nameEn}/>:<span>📦</span>}</div><div className="hunterScore">{score}<small>/100</small></div><h3>{p.nameEn}</h3><div className="meta">{p.threeCategoryName||p.twoCategoryName||p.oneCategoryName||'CJ Catalog'} · {Number(p.listedNum||0).toLocaleString()} listings</div><div className="metrics"><div><span>CJ cost</span><b>{money(cost)}</b></div><div><span>Suggested sell</span><b>{money(sell)}</b></div><div><span>Gross profit*</span><b>{money(profit)}</b></div><div><span>Margin*</span><b>{margin.toFixed(0)}%</b></div></div><div className="stockLine"><span>Verified stock: <b>{Number(p.totalVerifiedInventory||0).toLocaleString()}</b></span><span>{p.addMarkStatus===1?'Free shipping flag':'Freight available on analysis'}</span></div><div className="buttons"><Link className="btn light" href={`/admin/discover?q=${encodeURIComponent(p.nameEn)}`}>Similar</Link><Link className="btn dark" href={`/admin/discover/freight?pid=${encodeURIComponent(p.id)}&country=US`}>Analyze freight</Link></div></article>})}</section>
  <div className="panel roadmap"><h3>Connector status</h3><p className="meta">Product search, authentication and on-demand freight analysis are active. Live supplier orders and payments remain locked.</p><div className="roadSteps"><span className="pill ok">✓ CJ authentication</span><span className="pill ok">✓ Live product search</span><span className="pill ok">✓ Freight analyzer</span><span className="pill ok">✓ Secure server-side key</span><span className="pill warn">Import Draft next</span><span className="pill">Live orders locked</span></div><p className="meta" style={{marginTop:14}}>*Catalog-card profit and margin still exclude freight until you open Analyze freight. The freight page uses a real CJ quote for the selected product's first available variant.</p></div></div>
}
