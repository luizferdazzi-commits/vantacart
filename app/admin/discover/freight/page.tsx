...
export default async function FreightPage({searchParams}:{searchParams:Promise<{pid?:string;country?:string}>}){
  ...
  return <div className="main">
    ...
    {data&&<>
      <section className="panel freightProduct"><div className="freightHead">{data.variant?.variantImage?<img src={data.variant.variantImage} alt={data.variant?.variantNameEn||'CJ product variant'}/>:null}<div><div className="meta">{data.variant?.variantSku||pid}</div><h2>{data.variant?.variantNameEn||'CJ product variant'}</h2><div className="meta">VID: {data.variant?.vid} · Origin {data.origin} → {data.destination}</div></div></div></section>
