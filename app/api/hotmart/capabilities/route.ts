...
export async function GET(){
  try{
    ...
    for(const probe of probes){
      const r=await fetch(probe.endpoint,{headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},cache:'no-store'});
      const body=await r.json().catch(()=>null);
      results.push({name:probe.name,status:r.status,available:r.ok,count:Array.isArray(body?.items)?body.items.length:null,pageInfo:body?.page_info||null,error:r.ok?null:(body?.error_description||body?.error||body?.message||null)});
    }
    return NextResponse.json({ok:true,network:'hotmart',results});
  }catch(e:any){return NextResponse.json({ok:false,network:'hotmart',error:e?.message||'Hotmart capability probe failed'},{status:500});}
}
