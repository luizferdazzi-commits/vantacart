...
const copy={
  ...
  en:{search:'Search products, brands or tools',all:'All',deals:'Deals',ai:'AI & Software',business:'Business',home:'Home office',creators:'Creators',video:'Video & Design',heroBadge:'CURATED DEALS',heroTitle:'Find tools and products that are actually worth it.',heroText:'Approved partners, official links and organized offers so you can compare and choose faster.',heroCta:'See deals',featured:'Featured deals',featuredSub:'Active programs synchronized directly with our partners.',categories:'Explore by category',disclosure:'Some links are affiliate links. We may earn a commission at no extra cost to you.'}
} as const;

const catsPt=[['🤖','IA & Software','ai'],['📈','Negócios','business'],['⌨️','Home office','home'],['🎬','Criadores','creators'],['✨','Produtividade','productivity'],['💻','Tecnologia','technology']];
const catsEn=[['🤖','AI & Software','ai'],['📈','Business','business'],['⌨️','Home office','home'],['🎬','Creators','creators'],['✨','Productivity','productivity'],['💻','Technology','technology']];
  ...
  return <main className="cleanMarket marketHomeV2">
    ...
    <header className="mainHeader"><div className="mainHeaderInner">
      ...
      <form className="cleanSearch" action="/" method="get">
        ...
        <select aria-label="Categoria" name="category" defaultValue={category}><option value="all">{t.all}</option><option value="ai">{t.ai}</option><option value="business">{t.business}</option><option value="home">{t.home}</option><option value="creators">{t.creators}</option><option value="technology">{lang==='pt'?'Tecnologia':'Technology'}</option></select>
    ...
    <nav className="cleanNav"><div className="cleanNavInner">
      <span><Tag size={15}/> {t.deals}</span><Link href={linkFor('ai')}>{t.ai}</Link><Link href={linkFor('business')}>{t.business}</Link><Link href={linkFor('home')}>{t.home}</Link><Link href={linkFor('creators')}>{t.creators}</Link><Link href={linkFor('technology')}>{t.video}</Link>
    </div></nav>
