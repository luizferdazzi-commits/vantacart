...
const copy={
  pt:{search:'Buscar produtos, marcas ou ferramentas',all:'Todos',deals:'Ofertas',ai:'IA & Software',business:'Negócios',home:'Home office',creators:'Criadores',video:'Vídeo & Design',heroBadge:'OFERTAS SELECIONADAS',heroTitle:'Encontre ferramentas e produtos que realmente valem a pena.',heroText:'Parceiros aprovados, links oficiais e ofertas organizadas para você comparar e escolher mais rápido.',heroCta:'Ver ofertas',featured:'Ofertas em destaque',featuredSub:'Programas ativos sincronizados diretamente com nossos parceiros.',categories:'Explore por categoria',why:'Por que comprar pela VantaCart?',disclosure:'Alguns links são de afiliados. Podemos receber comissão sem custo adicional para você.'},
  en:{search:'Search products, brands or tools',all:'All',deals:'Deals',ai:'AI & Software',business:'Business',home:'Home office',creators:'Creators',video:'Video & Design',heroBadge:'CURATED DEALS',heroTitle:'Find tools and products that are actually worth it.',heroText:'Approved partners, official links and organized offers so you can compare and choose faster.',heroCta:'See deals',featured:'Featured deals',featuredSub:'Active programs synchronized directly with our partners.',categories:'Explore by category',why:'Why shop through VantaCart?',disclosure:'Some links are affiliate links. We may earn a commission at no extra cost to you.'}
} as const;

const catsPt=[['🤖','IA & Software'],['📈','Negócios'],['⌨️','Home office'],['🎬','Criadores'],['✨','Produtividade'],['💻','Tecnologia']];
const catsEn=[['🤖','AI & Software'],['📈','Business'],['⌨️','Home office'],['🎬','Creators'],['✨','Productivity'],['💻','Technology']];
...
export default async function Home({searchParams}:{searchParams:Promise<SearchParams>}){
  ...
  const cats=lang==='pt'?catsPt:catsEn;
  ...
  return <main className="cleanMarket marketHomeV2">
    ...
    <header className="mainHeader"><div className="mainHeaderInner">
      ...
      <form className="cleanSearch" action="#deals">
        <select aria-label="Categoria"><option>{t.all}</option><option>{t.ai}</option><option>{t.business}</option><option>{t.home}</option></select>
    ...
    <nav className="cleanNav"><div className="cleanNavInner">
      <span><Tag size={15}/> {t.deals}</span><a href="#deals">{t.ai}</a><a href="#deals">{t.business}</a><a href="#deals">{t.home}</a><a href="#deals">{t.creators}</a><a href="#deals">{t.video}</a>
    </div></nav>
    ...
    <section className="marketSection" id="deals">
      ...
      <ImpactCampaignGrid lang={lang}/>

      <div className="marketSectionHead"><div><h2>{t.categories}</h2></div></div>
      <div className="marketCategoryBar">{cats.map(([icon,name])=><a href="#deals" key={name}><span>{icon}</span>{name}</a>)}</div>

      <div className="marketTrust">
        <div><ShieldCheck size={22}/><span><b>{lang==='pt'?'Parceiros verificados':'Verified partners'}</b><p>{lang==='pt'?'Só mostramos programas ativos e aprovados.':'We only surface active, approved programs.'}</p></span></div>
        <div><Zap size={22}/><span><b>{lang==='pt'?'Acesso direto':'Direct access'}</b><p>{lang==='pt'?'Você continua no site oficial do parceiro.':'You continue on the partner’s official website.'}</p></span></div>
        <div><Tag size={22}/><span><b>{lang==='pt'?'Sem taxa adicional':'No extra fee'}</b><p>{lang==='pt'?'A indicação não aumenta o preço para você.':'Our referral does not increase your price.'}</p></span></div>
      </div>
    </section>
