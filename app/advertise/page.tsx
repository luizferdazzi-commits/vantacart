...
export default function AdvertisePage() {
  return <main className="vendorPage">
    ...
    <section className="vendorSection">
      <div className="vendorHead"><h2>Veja o formato, o preço e contrate no mesmo lugar</h2><p>Cada opção abaixo mostra como sua marca aparece na VantaCart, o que está incluído e o botão direto para pagamento seguro no Stripe.</p></div>
      <div className="offerGrid">{plans.map(plan=><article key={plan.name} className={`offerCard ${plan.tone}${plan.recommended?' recommended':''}`}>
        ...
        <div className="offerBody"><PreviewMockup type={plan.type}/><div className="offerCommercial"><div className="offerPrice"><b>{plan.price}</b><span>{plan.cadence}</span></div><p>{plan.description}</p><ul>{plan.features.map(feature=><li key={feature}><CheckCircle2 size={14}/>{feature}</li>)}</ul><a className="offerCta" href={plan.href} target="_blank" rel="noopener noreferrer sponsored">{plan.cta}</a></div></div>
      </article>)}</div>
      <div className="vendorInfo"><article><Megaphone size={22}/><h3>Sponsored visibility</h3><p>Paid placements are labeled Sponsored so buyers can distinguish commercial exposure from editorial recommendations.</p></article><article><TrendingUp size={22}/><h3>High-intent surfaces</h3><p>Plans are designed for category pages, comparisons, launch exposure and other decision-stage experiences.</p></article><article><ShieldCheck size={22}/><h3>Affiliate-compatible</h3><p>Where program terms allow it, vendor sponsorship and affiliate tracking can coexist as separate commercial relationships.</p></article></div>
