...
let cachedToken:string|undefined; let cachedUntil=0;
async function getAccessToken(){const now=Date.now();if(cachedToken&&now<cachedUntil)return cachedToken;const apiKey=process.env.CJ_API_KEY;if(!apiKey)throw new Error('CJ_API_KEY is not configured');const res=await fetch(`${CJ_BASE}/authentication/getAccessToken`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({apiKey}),cache:'no-store'});const json=await res.json();if(!res.ok||!json?.result||!json?.data?.accessToken)throw new Error(json?.message||'Unable to obtain CJ access token');cachedToken=String(json.data.accessToken);const expiry=Date.parse(json.data.accessTokenExpiryDate||'');cachedUntil=Number.isFinite(expiry)?Math.max(now+60000,expiry-300000):now+3600000;return cachedToken;}
async function cjJson(url:string|URL,init:RequestInit={}){const token=await getAccessToken();const headers=new Headers(init.headers||{});headers.set('CJ-Access-Token',token);const res=await fetch(url,{...init,headers,cache:'no-store'});const json=await res.json();if(!res.ok||!json?.result)throw new Error(json?.message||'CJ API request failed');return json;}
export type CjProduct={id:string;nameEn:string;sku?:string;spu?:string;bigImage?:string;sellPrice?:string;nowPrice?:string;listedNum?:number;oneCategoryName?:string;twoCategoryName?:string;threeCategoryName?:string;warehouseInventoryNum?:number;totalVerifiedInventory?:number;supplierName?:string;deliveryCycle?:string;addMarkStatus?:number};
