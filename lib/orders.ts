...
export type OrderItemInput={id:string;name:string;price:number;qty:number;vid?:string;variant?:string};

function sqlClient(){const url=process.env.DATABASE_URL;if(!url)throw new Error('DATABASE_URL is not configured');return neon(url);}

export async function ensureOrders(){
  const sql=sqlClient();
  await sql`CREATE TABLE IF NOT EXISTS orders (
    id BIGSERIAL PRIMARY KEY, public_id TEXT UNIQUE NOT NULL, stripe_session_id TEXT UNIQUE,
    stripe_payment_intent_id TEXT, payment_status TEXT NOT NULL DEFAULT 'PENDING',
    fulfillment_status TEXT NOT NULL DEFAULT 'AWAITING_PAYMENT', currency TEXT NOT NULL DEFAULT 'usd',
    subtotal NUMERIC(12,2) NOT NULL DEFAULT 0, shipping_amount NUMERIC(12,2) NOT NULL DEFAULT 0,
    total_amount NUMERIC(12,2) NOT NULL DEFAULT 0, destination_country TEXT, destination_postal_code TEXT,
    shipping_method TEXT, customer_email TEXT, customer_name TEXT, customer_phone TEXT,
  ...
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS cj_payload JSONB`;
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS fulfillment_error TEXT`;
  ...
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS fulfilled_at TIMESTAMPTZ`;
  await sql`CREATE TABLE IF NOT EXISTS order_items (
    ...
    cj_product_id TEXT NOT NULL, cj_variant_id TEXT, product_name TEXT NOT NULL, variant_name TEXT,
...
export async function createPendingOrder(input:{items:OrderItemInput[];shipping:number;country:string;zip:string;shippingMethod:string}){
  await ensureOrders();const sql=sqlClient();const subtotal=input.items.reduce((sum,item)=>sum+(item.price*item.qty),0);const total=subtotal+input.shipping;
  ...
  for(const item of input.items)await sql`INSERT INTO order_items (order_id,cj_product_id,cj_variant_id,product_name,variant_name,unit_price,quantity)
    VALUES (${order.id},${item.id},${item.vid||null},${item.name},${item.variant||null},${item.price},${item.qty})`;
  return order;
}

export async function attachStripeSession(publicId:string,sessionId:string){await ensureOrders();const sql=sqlClient();await sql`UPDATE orders SET stripe_session_id=${sessionId},updated_at=NOW() WHERE public_id=${publicId}`;}
export async function markOrderCheckoutFailed(publicId:string,payload:unknown){await ensureOrders();const sql=sqlClient();await sql`UPDATE orders SET payment_status='CHECKOUT_FAILED',stripe_payload=${JSON.stringify(payload)},updated_at=NOW() WHERE public_id=${publicId}`;}

export async function markOrderPaidFromSession(session:any){
  await ensureOrders();const sql=sqlClient();const publicId=String(session?.metadata?.order_id||session?.client_reference_id||'');const sessionId=String(session?.id||'');
...
export async function markOrderCjSubmitted(publicId:string,result:any){
  await ensureOrders();const sql=sqlClient();const data=result?.data||{};
  const cjOrderId=String(data?.orderId||data?.id||data?.orderNum||'')||null;
  const cjOrderNumber=String(data?.orderNumber||data?.orderNum||publicId);
  await sql`UPDATE orders SET fulfillment_status='SUBMITTED_TO_CJ',cj_order_id=${cjOrderId},cj_order_number=${cjOrderNumber},cj_request_id=${result?.requestId||null},cj_payload=${JSON.stringify(result)},fulfilled_at=NOW(),updated_at=NOW() WHERE public_id=${publicId}`;
}

export async function markOrderFulfillmentFailed(publicId:string,error:string){await ensureOrders();const sql=sqlClient();await sql`UPDATE orders SET fulfillment_status='FULFILLMENT_FAILED',fulfillment_error=${error.slice(0,2000)},updated_at=NOW() WHERE public_id=${publicId}`;}

export async function getOrderBySession(sessionId:string){await ensureOrders();const sql=sqlClient();const rows=await sql`SELECT public_id,payment_status,fulfillment_status,total_amount,currency,customer_email,created_at,paid_at FROM orders WHERE stripe_session_id=${sessionId} LIMIT 1`;return rows[0] as any|undefined;}

export async function listOrders(limit=50){
  await ensureOrders();const sql=sqlClient();
  const rows=await sql`SELECT o.id,o.public_id,o.payment_status,o.fulfillment_status,o.total_amount,o.currency,o.customer_email,o.customer_name,o.destination_country,o.destination_postal_code,o.shipping_method,o.cj_order_id,o.cj_order_number,o.fulfillment_error,o.tracking_number,o.tracking_url,o.created_at,o.paid_at,o.fulfilled_at,
